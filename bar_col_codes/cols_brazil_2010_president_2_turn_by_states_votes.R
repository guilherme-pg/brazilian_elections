# BRAZIL 2010 presidential election 2 turn by state votes
...
#votos_2010 <- read.table("../../general_election_data/votacao_secao_2010_BR.csv", sep=";", header=TRUE)
#votos_presidente = filter(votos_2010, NR_TURNO==2)
#rm(votos_2010)
#write.csv(votos_presidente, "../../general_election_data/brazil_2010_president_2_turn.csv", row.names = FALSE)
...
# LOADING DATASETS
# VOTES
votos_br <- read.table("../../general_election_data/brazil_2010_president_2_turn.csv", header=TRUE, sep=",")
...
# CLEAN
votos_pr_2 <- votos_pr_2[!(votos_pr_2$NM_VOTAVEL=="VOTO ANULADO E APURADO EM SEPARADO"), ]
...
greatest_states %>%
  ...
  labs(y = "Votos",
       title = "Votos Válidos no 2º Turno das Eleições para Presidente de 2010",
...
# save last plot
ggsave("f1_col_president_2_turn_2010_br_by_greatest_state.jpg")
