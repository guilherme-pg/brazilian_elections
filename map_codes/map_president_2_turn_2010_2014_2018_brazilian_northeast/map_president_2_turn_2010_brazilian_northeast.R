# Brazilian Northeast 2010 elections PRESIDENT 2 turn
...
#votos_estados <- filter(votos_2010, NR_TURNO==2 & SG_UF %in% c("AL", "BA", "CE", "MA", "PB", "PE", "PI", "RN", "SE"))
#write.csv(votos_estados, "df_ne_2010_president_2_turn.csv", row.names = FALSE)
...
# VOTES
votos_NE <- read.table("../../../general_election_data/df_ne_2010_president_2_turn.csv", header=TRUE, sep=",")
...
votos_validos %>%
  ...
  labs(
    title = "Votos Válidos no 2º Turno das Eleições para Presidente de 2010 no Nordeste",
...
ggsave("map_president_2_turn_2010_ne.jpg")
