...
# VOTES
votos_2018 <- read.table("../general_election_data/votacao_secao_2018_BR.csv", header=TRUE, sep=";")

# MAPS
MAPA_MG <- st_read("31MUE250GC_SIR.shp")
MAPA_RJ <- st_read("33MUE250GC_SIR.shp")
MAPA_SP <- st_read("35MUE250GC_SIR.shp")
...
votos_mg <- filter(votos_2018, NR_TURNO==2 & SG_UF =="MG")
votos_rj <- filter(votos_2018, NR_TURNO==2 & SG_UF =="RJ")
votos_sp <- filter(votos_2018, NR_TURNO==2 & SG_UF =="SP")

votos_2018_pr_2_t <- rbind(votos_mg, votos_rj, votos_sp)

votos_pr_2 <- votos_2018_pr_2_t %>%
...
# ADD UF'S
df_votos_validos$SG_UF <- votos_2018_pr_2_t$SG_UF[ match( df_votos_validos$CD_MUNICIPIO,  votos_2018_pr_2_t$CD_MUNICIPIO) ]
